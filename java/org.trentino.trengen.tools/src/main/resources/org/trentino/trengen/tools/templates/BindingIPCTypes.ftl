...
#include <Binding/IPC/TrentinoBindingIpcMemoryMapObj.h>

<#foreach include in includes>
#include "${include}"
</#foreach>

<#assign isThereAtLeastOneComplexType = 0>
...
<#foreach field in type.fields>
	...
	<#elseif tmapper.isComplexType(field.type)==true && j== -1 && i==-1>
		...
		<#assign isThereAtLeastOneComplexType = 1>
...
<#macro traceComplexParamsConstruct type>
<#foreach field in type.fields>
	<#assign name=tmapper.name(field.getType())/>
	<#assign j = name?index_of("string")>
	<#assign i = name?index_of("char*")>
	...
	<#if tmapper.isComplexType(field.type)==true && j== -1 && i==-1>
		<#assign name=field.className/>
		<#assign isThereAtLeastOneComplexType = 1>
		//construct pointer type in shared memory
		mManaged_shm->destroy<IPC${name}>("ipc_${name}_${field.name}");
		${field.name}_ = mManaged_shm->construct<IPC${name}>("ipc_${name}_${field.name}")(mManaged_shm);
	</#if>
</#foreach>
</#macro>

<#macro traceComplexParamsDestroy type>
<#foreach field in type.fields>
	<#assign name=tmapper.name(field.getType())/>
	<#assign j = name?index_of("string")>
	<#assign i = name?index_of("char*")>
...
public:
		...
		IPC${classn}( boost::interprocess::managed_shared_memory* managed_shm)
		:mManaged_shm(managed_shm)
		{
			<@traceComplexParamsConstruct type= typeDesc/>
		};

		~IPC${classn}()
		{
			<#if isThereAtLeastOneComplexType == 1>
			<@traceComplexParamsDestroy type=typeDesc />
			</#if>
		};
...
<#if typeDesc.enclosed >
<#assign classn=typeDesc.className/>
inline IPC${typeDesc.className}* populate_IPC${typeDesc.className}(${typeDesc.namespace}::${typeDesc.className}* value, boost::interprocess::managed_shared_memory& managed_shm)
{
...
class IpcMemoryMapObj_${method.name}: public Trentino::Binding::IPC::IpcMemoryMapObj {
	...
	public:
	    ...
	    IpcMemoryMapObj_${method.name}( boost::interprocess::managed_shared_memory* managed_shm)
		:mManaged_shm(managed_shm)
		{
		////////////////////////////
			<#foreach arg in method.arguments>
	 			...
	 			<#if j!= -1 || i!=-1 >
	 			//yyy
				mManaged_shm->destroy<${tmapper.name(arg.type)}> ("ipc_${tmapper.name(arg.type)}_${arg.name}");
				"ipc_${class}_${field.name}"
				${arg.name}_ =  mManaged_shm->construct<${tmapper.name(arg.type)}>("ipc_${tmapper.name(arg.type)}_${arg.name}")(mManaged_shm);
				</#if>
				<#if (tmapper.isPrimitive(arg.type)==true && j== -1 && i==-1)||arg.type.enumeration>
				
				<#elseif tmapper.isComplexType(arg.type)==true && j== -1 && i==-1>
				//xxx
				mManaged_shm->destroy<IPC${arg.type.className}> ("ipc_${arg.type.className}_${arg.name}");
				
				${arg.name}_ =  mManaged_shm->construct<IPC${arg.type.className}>("ipc_${arg.type.className}_${arg.name}")(mManaged_shm);
				</#if>
		</#foreach>
		//return values	
		<#foreach argRet in method.returnArguments>
