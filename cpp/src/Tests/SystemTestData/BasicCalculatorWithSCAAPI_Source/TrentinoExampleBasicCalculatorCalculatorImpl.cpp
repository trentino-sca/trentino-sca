...
int CalculatorImpl::div2(int a, int b)
{
   ...
   //we are using ComponentContext::getCurrent here. setComponentContext in Impl is invasive...
   //it is a little bit unclear why we don´t use the API conform way?
   ServiceProxyPtr service = ComponentContext::getCurrent()->getService("divider");
	Divider* divider = dynamic_cast<Divider*>(service.get());
