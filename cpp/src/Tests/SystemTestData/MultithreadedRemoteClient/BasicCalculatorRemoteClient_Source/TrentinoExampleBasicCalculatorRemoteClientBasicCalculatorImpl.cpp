...
//services
	...
	void BasicCalculatorRemoteClientImpl::thread1() 
	{ 
		// addSimple
		
		int addSimpleRet = m_basicCalc->addSimple(3, 5);
		mutex.lock(); 
		std::cout << "Thread " << boost::this_thread::get_id() <<"addSimple returned " << addSimpleRet << std::endl;
		mutex.unlock(); 
		...
		mutex.lock(); 
		std::cout << "Thread " << boost::this_thread::get_id() <<"add returned " << addRet.a() <<" "<<addRet.b() << std::endl;
		mutex.unlock(); 
		...
		mutex.lock(); 
		std::cout << "Thread " << boost::this_thread::get_id() <<"multiply returned " << cn3->a() <<" "<<cn3->b() << std::endl;
		mutex.unlock(); 

		// divide
		ComplexNumber divideRet = m_basicCalc->divide(*cn1, *cn2);
		mutex.lock(); 
		std::cout << "Thread " << boost::this_thread::get_id() <<"divide returned " << divideRet.a() <<" "<<divideRet.b() << std::endl;
		mutex.unlock(); 
		...
		mutex.lock(); 
		std::cout << "Thread " << boost::this_thread::get_id() <<"divide2 returned " << divide2Ret.a() <<" "<<divide2Ret.b() << std::endl;
		mutex.unlock(); 
		...
		// recursive Person
		Person* mother = child->mutable_mother();
		mother->set_name("merry");
		mother->set_surname("tatli");
	
		std::string ret=  mother->has_mother() ?  mother->mutable_mother()->name() : mother->name();
		mutex.lock(); 	
		std::cout << "Thread " << boost::this_thread::get_id()  << ret << std::endl;
		mutex.unlock(); 
	} 
	 ...
	 void BasicCalculatorRemoteClientImpl::setBasicCalculatorRef(BasicCalculator* basicCalc)
	 {
		...
		boost::thread myThread(boost::bind(&BasicCalculatorRemoteClientImpl::thread1, this));
		boost::thread myThread2(boost::bind(&BasicCalculatorRemoteClientImpl::thread1, this));
		boost::thread myThread3(boost::bind(&BasicCalculatorRemoteClientImpl::thread1, this));
		myThread.join();
		myThread2.join();
		myThread3.join();	
