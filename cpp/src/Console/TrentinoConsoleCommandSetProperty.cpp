...
using Trentino::Console::CommandSetProperty;
...
//class CommandSetProperty
   //construction
CommandSetProperty::CommandSetProperty( const CommandArgs& commandArgs
...
//services
void CommandSetProperty::acceptOutputVisitor(Trentino::Console::OutputVisitor& visitor)
{
   visitor.visitCommandSetProperty(this);
}

void CommandSetProperty::execute()
{
   ...
   const string& propertyURI = commandArgs().propertyURI;
   const vector<string>& values = commandArgs().values;

   setErrorMessage(iface->setProperty(propertyURI, values));
}
