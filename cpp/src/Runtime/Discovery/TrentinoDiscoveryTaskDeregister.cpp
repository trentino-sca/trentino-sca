...
DiscoveryTaskDeregister::DiscoveryTaskDeregister(discoveryFrameWorkCBFuncPtr funcPtr, 
   int retryCount,
   int retryInterval,
...
bool DiscoveryTaskDeregister::execute()
{
   mDiscoveryMethodInterface->deregisterSrv(mTaskPtr);
   if(mStatus)
   {
      logInfo(mReturnValue.c_str());
      // No mRuntimeCB is triggered for deregister
      return true;
   }
   else
   {
